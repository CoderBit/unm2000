...
const apiUrl = 'http://localhost:8084/unm2000/fileManagement/';
...
export class FolderCompareComponent implements OnInit {
  ...
  newTreeAfter = [];
  childAfter = [];
  listOfMapData = this.newTree;
  listOfMapDataAfter: TreeNodeInterface[] = this.newTreeAfter;
  mapOfExpandedData: { [key: string]: TreeNodeInterface[] } = {};
  mapOfExpandedDataAfter: { [key: string]: TreeNodeInterface[] } = {};
  ...
  ngOnInit() {
    ...
    // console.log(JSON.stringify(this.listOfMapData));
    // this.listOfMapData = this.asdf;
    ...
    this.http.get<{ code: number, data: any, errorMessage: string, message: string, totalData: string }>(apiUrl + 'compareHistory')
      .subscribe(
        res => {
          if (res.code === 1000) {
            console.log(res);
            this.resultpath = res.data[0].reslutRootPath;
            this.sourcepath = res.data[0].sourceBeforePath;
            this.targetpath = res.data[0].sourceAfterPath;
            // this.filterBean = res.data[0].sourceAfterPath
    ...
    const interval = setInterval(() => {
      this.setTimeOutFlagshow = false;

      this.http.get<{ code: number, data: any, errorMessage: string, message: string, totalData: string }>(apiUrl + 'compareProgress')
        .subscribe(
          res => {
            if (res.code === 1000) {
              this.comparedisabled = true;
              this.setTimeOutFlagshow = true;
              this.progressStatus = res.data;
              this.copyPerc = Math.floor((this.progressStatus.finishCopyFileCount/this.progressStatus.totalCopyFileCount)*100);
              this.compPerc = Math.floor((this.progressStatus.finishCompareFileCount/this.progressStatus.totalCompareFileCount)*100);

              console.log('this.copyPerc',this.progressStatus.finishCopyFileCount, this.copyPerc);
              console.log('this.progressStatus', this.progressStatus);
              // tslint:disable-next-line:max-line-length
              if ((res.data.copyProgressStatus === 'finish' || res.data.copyProgressStatus === 'stopped' || res.data.copyProgressStatus === 'notstart') && res.data.compareProgressStatus === 'finish' || res.data.compareProgressStatus === 'stopped' || res.data.compareProgressStatus === 'notstart') {
  ...
  compare() {
    ...
    if (this.sourcepath == null || this.resultpath == null || this.targetpath == null || this.sourcepath === '' || this.resultpath === '' || this.targetpath === '') {
          ...
          .subscribe(
            res => {
              if (res.code === 1000) {
                ...
                console.log('this.progressStatus', this.progressStatus);
                // tslint:disable-next-line:max-line-length
                if ((res.data.copyProgressStatus === 'finish' || res.data.copyProgressStatus === 'stopped') && res.data.compareProgressStatus === 'finish' || res.data.compareProgressStatus === 'stopped') {
                  clearInterval(interval);
                  this.comparedisabled = false;
      ...
      this.http
        .post<{ code: number, }>(apiUrl + 'compareFolders', body)
        .subscribe(res => {
          if (res.code === 1000) {
  ...
  getResult() {
    ...
    if (this.sourcepath == null || this.resultpath == null || this.targetpath == null || this.sourcepath === '' || this.resultpath === '' || this.targetpath === '') {
      ...
      this.http
        .post<{ code: number; data: any; errorMessage: string; message: string }>(apiUrl + 'getResult', body)
        .subscribe(
          res => {
            if (res.code === 1000) {
              ...
              this.listOfMapData = this.recurr(res.data.before)[0];
              this.listOfMapData.forEach(item => {
                this.mapOfExpandedData[item.key] = this.convertTreeToList(item);
              });
              console.log("this.listOfMapData", this.listOfMapData);
              console.log("this.listOfMapData", JSON.stringify(this.listOfMapData));
              this.listOfMapDataAfter = this.recurr(res.data.after)[0];
              this.listOfMapDataAfter.forEach(item => {
                this.mapOfExpandedDataAfter[item.key] = this.convertTreeToList(item);
              });
              console.log("this.listOfMapDataAfter", this.listOfMapDataAfter);
        ...
        .subscribe(
          res => {
            if (res.code === 1000) {
              ...
              this.progressStatus = res.data;
              // console.log('this.progressStatus', this.progressStatus);
              // const copyProgressStatus = res.data.copyProgressStatus
              // const compareProgressStatus = res.data.compareProgressStatus
              // const totalCopyFileCount = res.data.totalCopyFileCount
              // const finishCopyFileCount = res.data.finishCopyFileCount
              // const totalCompareFileCount = res.data.totalCompareFileCount
              // const finishCompareFileCount = res.data.finishCompareFileCount
  ...
  stopCompare() {
    ...
    this.http.get<{ code: number, data: any, errorMessage: string, message: string, totalData: string }>(apiUrl + 'stop')
      .subscribe(
        res => {
          if (res.code === 1000) {
  ...
  recurr(data) {
    ...
    for (const file of data.Files) {
      const inputFile = {
        key: file.fileName + '-' + this.randomKey,
        name: file.fileName,
    ...
    for (const directory of data.Directory) {
      if (!directory.hasOwnProperty('Directory')) {
        ...
        const p = {
          key: directory.directoryName + '-' + this.randomKey,
  ...
  insertFolderData(item) {
    ...
    item.Directory.forEach(newditem => {
      ...
      if (hasFolders || hasFiles) {
        ...
        if (pushFolderDataParent.length > 0) {
          ...
          this.newTree[index].children.push({
            key: newditem.directoryName,
            name: newditem.directoryName,
            age: newditem.directorySize,
            address: newditem.lastModifiedTime,
            type: 'folder',
            parentName: newditem.parentName,
            children: []
          });
        } else {
          this.newTree.push({
    ...
    item.Files.forEach(newfitem => {
      ...
      console.log('ab', ab);
      const pushDataParent = this.newTree.filter(aaab => {
        return (aaab.type === 'folder' && aaab.name === fileParent);
      });
      if (pushDataParent.length > 0) {
        const index = this.newTree.indexOf(pushDataParent[0]);
        this.newTree[index].children.push({
          key: newfitem.fileSize,
          name: newfitem.fileName,
          age: newfitem.fileSize,
          address: newfitem.lastModifiedTime,
          type: 'file'
        });
      } else {
        this.newTree.push({
  ...
  insertFolderDataAfter(item) {
    ...
    item.Directory.forEach(newditem => {
      ...
      if (hasFolders || hasFiles) {
        ...
        if (pushFolderDataParent.length > 0) {
          ...
          console.log(pushFolderDataParent, index);
          this.newTreeAfter[index].children.push({
            key: newditem.directoryName,
            name: newditem.directoryName,
            age: newditem.directorySize,
            address: newditem.lastModifiedTime,
    ...
    item.Files.forEach(newfitem => {
      ...
      console.log('ab', ab);
      const pushDataParent = this.newTreeAfter.filter(aaab => {
        return (aaab.type === 'folder' && aaab.name === fileParent);
      });
      if (pushDataParent.length > 0) {
